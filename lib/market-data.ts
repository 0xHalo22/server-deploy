...
async function fetchBirdseyeKlines(symbol: string, interval: string, limit: number = 1000): Promise<MarketData[]> {
  try {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        const transformedData = data.data.ohlcv.map((item: any) => ({
          symbol,
